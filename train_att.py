...
if __name__ == "__main__":
    ...
    model = Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone, att=True).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
    ...
    #   读取数据集对应的txt
    # ---------------------------#
    with open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/trainval.txt"), "r") as f:
